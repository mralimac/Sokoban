...
public class Gameplay{
	...
	public Gameplay()
	{
		...
		{
			numberOfFiles = 0;
			alertHandler.newError("Warning No Level files are present\nExiting Sokoban...");			
			System.exit(0);
		}
		this.playerID = Setup.getPlayerID();
		this.winHandle = new WinHandler();
	 ...
	 * 
	 * @param gridPaneY Stores the Y Coord for the gridpane
	 * @param loadLevelLable The Title of the Menu
	 * @param scoreboardJSON The JSON object gathered from the API call
	 * 
	 */
	private void generateMenu()
	{
		
		Setup.getStage().setWidth(getWidthOfWindow());
	    Setup.getStage().setHeight(getHeightOfWindow());
		resetAll();
		Setup.getBorderPane().setCenter(null);
		Setup.getBorderPane().setLeft(Setup.getGridPane());
		...
		Setup.getGridPane().getChildren().add(loadLevelLabel);
		...
		try {
			...
			Setup.getBorderPane().setRight(scoreBoardGrid);
		} catch (JSONException e) {
			e.printStackTrace();
			alertHandler.newError("JSON Malformed");
		}
		...
		for(int i = 0; i < listOfFiles.size(); i++)
		{
			...
			button.setOnMouseClicked(new EventHandler<MouseEvent>()
	        {
	            ...
	            public void handle(MouseEvent t) {
	            	...
	            	{
	            		alertHandler.newError("File name is incorrect. Please rename to 'level[X].txt' where the [x] is the levels number\neg:Level1.txt");
	            	} catch (IOException e) {
	            		alertHandler.newError("Problem reading the file");
						e.printStackTrace();
					}
			...
			Setup.getGridPane().getChildren().addAll(button);
		}
	 ...
	 * @param urlObject This is the URL string that the API calls
	...
	private JSONObject callAPI(URL urlObject) throws IOException
	{
		...
		if(responseCode != 200)
		{
			...
			alertHandler.newError("API miscommuncation occured");
		}
	 ...
	 * @return This returns the JSONObject for this specific API call
	 */
	...
	/**
	 * @param levelNumber The level the player was on when they completed the game
	 * @param levelScore The score they achieved
	 * @throws IOException Thrown if the JSON is malformed
	 */
	...
	private void generateWinScreen()
	{
		...
		new Thread(() -> { 
			try {
				...
				e1.printStackTrace();
				alertHandler.newError("Failed to update API");
			}
		}).start();
		...
		Setup.getBorderPane().setTop(labelBox);
		...
		Setup.getBorderPane().setCenter(boxOfButtons);
	 ...
	 * @param buttonTitle Label to put on the button
	 * @param doThisOnClick The eventhandler specifiying what actions to take
	 ...
	 * @param levelToLoad The number of the level to load
	 * @throws IOException Thrown if the level cannot be loaded
	 */
	private void loadLevel(int levelToLoad) throws IOException
	{
		resetAll();
		this.stepsTaken = 0;
		this.currentLevel = new Level(levelToLoad, listOfFiles.get(levelToLoad-1).getAbsolutePath(), winHandle);
		Setup.getStage().setWidth(getWidthOfWindow());
	    Setup.getStage().setHeight(getHeightOfWindow());
	    ...
	    Setup.getBorderPane().setTop(topLayer);
	 ...
	 * @return Returns the width of the window in pixels
	 */
	 ...
	 * If there is a level in play, it makes a window relevant to the level, if not then it generates a static window based
	 * @return Returns the height of the window in pixels
	 */
	 ...
	 * @return Returns the arraylist of files
	 */
	 ...
	 * @return Returns the current level loaded
	 */
	...
	/**
	 * This function wipes out the grid and winHandler
	 */
	private void resetAll()
	{
		Setup.getGridPane().getChildren().clear();
		winHandle.clearAll();
		Setup.getGridPane().setVgap(0);
		Setup.getGridPane().setHgap(0);
		Setup.getBorderPane().setTop(null);
		Setup.getBorderPane().setCenter(null);
		Setup.getBorderPane().setRight(null);
		Setup.getBorderPane().setLeft(null);
		Setup.getBorderPane().setBottom(null);
		Setup.getBorderPane().setCenter(Setup.getGridPane());
	}
